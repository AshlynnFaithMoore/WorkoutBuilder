...
struct Exercise: Identifiable, Codable, Hashable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // Handle equipment as either string or array
        if let equipmentArray = try? container.decode([String].self, forKey: .equipment) {
            equipment = equipmentArray
        } else if let equipmentString = try? container.decode(String.self, forKey: .equipment) {
        ...
        // Handle category - try to decode from JSON first, then map from primary muscles
        if let categoryString = try? container.decode(String.self, forKey: .category) {
            category = ExerciseCategory(rawValue: categoryString) ?? ExerciseCategory.fromPrimaryMuscle(primaryMuscles.first ?? "")
        } else {
